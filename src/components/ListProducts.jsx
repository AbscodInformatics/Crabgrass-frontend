...
useEffect(()=>{
  // console.log(search)
  setData(apiData.filter((item)=>(item.product_name).toLowerCase().includes(search)))
 },[search])
            ...
            <div className="md:flex items-center mt-6 lg:mt-0">
              ...
              <div className="flex items-center mt-4 md:mt-0 md:ml-3 lg:ml-0">
                <div className="w-40 py-2 px-3 bg-white lg:ml-3 border rounded border-gray-200">
                  <select className="w-full text-sm leading-3 text-gray-500 focus:outline-none" onChange={(e)=>setSort(e.target.value)}>
              ...
              {data.map((item, id) => {
                return (
                  ...
                  >
                    ...
                    <td>
                      <div className="flex items-center">
                        <button onClick={()=>navigate(`/product/${item._id}`)} className="bg-gray-100 mr-3 hover:bg-blue-400 py-2.5 px-5 rounded text-sm leading-3 text-gray-500 focus:outline-none">
                          Edit
                        </button>
                        <button
